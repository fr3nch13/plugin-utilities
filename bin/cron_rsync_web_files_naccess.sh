...
# Sync the /var/www
time_start=`date +%s`
ECHOMSG="[$$][`date`]: Syncing naccess web files. Starting on: `date`"
fappend $EMAILBODY "${ECHOMSG}\n"
echo ${ECHOMSG}

/usr/bin/rsync -raz -e 'ssh -p 9001' --delete /var/www/naccess/app/webroot/files user@example.com:/var/www/naccess/app/webroot/files
RETVAL=$?
...
ECHOMSG="[$$][`date`]: An Error occurred while syncing naccess web files. Return code was: ${RETVAL} - in ${time_diff} seconds"
ERR_WEB=" - ERROR WEB "
else
ECHOMSG="[$$][`date`]: Completed Syncing naccess web files in ${time_diff} seconds"
fi
fappend $EMAILBODY "${ECHOMSG}\n"
echo ${ECHOMSG}
...
SUBJECT="Cron Rsync - naccess - ${ERR_WEB}- ${ECHOMSG}"
